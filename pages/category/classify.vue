	...
	<view class="page-body" :style="'height:'+height+'px'">
		...
		<scroll-view class="nav-right" scroll-y :scroll-top="scrollTop" @scroll="scroll" :style="'height:'+height+'px'" scroll-with-animation>
			<view v-for="(goods,index) in classifyData" :key="index" class="box">
				<view :id="i==0?'first':''" class="nav-right-item" v-for="(item,i) in goods.goods" :key="i" @click="cart(item.id)">
					<image :src="item.icon" />
					<view>{{item.name}}</view>
				</view>
			</view>
...
<script>
	export default {
		...
		methods: {
			loadInfo() {
				var that = this;
				this.$http.get(this.$httpApi.goods.classify, {}).
				then(function(response) {
					that.classifyData = response;
					that.$nextTick(()=>{
						that.getHeightList();
					})
	...
	// 					rect.height  // 节点的高度
			...
			cart: function (id) {
				uni.navigateTo({
					url: `/pages/product/list?classId=${id}`
				})
			}
		},
		components: {
