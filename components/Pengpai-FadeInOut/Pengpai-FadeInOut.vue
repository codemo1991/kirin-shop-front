...
export default {
	...
	created() {
		...
		let timer = setInterval(() => {
			if (!this.loop && count >= this.list.length - 2) clearInterval(timer);
			if (count >= this.list.length - 1) count = -1;
			if (this.list.length > 0 && count <= this.list.length) {
				count++;
				this.content = this.list[count];
				this.donghua();
			}
